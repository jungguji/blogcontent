...
slug: scalable-push-system-architecture
tags:
  - System Design
  - Architecture
  - Push Notification
  ...
  - Scalability
  - Retrospective
description: 19만 푸시 발송에 5시간 걸리던 레거시 시스템의 한계를 분석하고, 메시지 큐(Kafka), 아웃박스 패턴, Redis를 활용하여 5분 이내 발송 가능한 고성능 분산 아키텍처를 설계한 경험을 공유합니다. 안정성과 확장성, 데이터 정합성까지 확보하는 과정을 담았습니다.
---
...
graph TD
    ...
    PrepSchedulers -- "3. Prepare in Outbox" --> OutboxTable
    
    DispatchPublishers -- "4. Claim & Publish" --> OutboxTable
    DispatchPublishers --> MQ

    MQ --> Consumers

    Consumers -- "6. Final Check" --> RedisCache
    Consumers -- "7. Send Push (Success)" --> PushService

    %% Data Flow (Failure Path)
    Consumers -.->|"8. Final Failure <br/> (After Retries)"| DLQ
    ...
    style PushService fill:#eee,stroke:#333,stroke-width:2px
    style DLQ fill:#ffb3b3,stroke:#333,stroke-width:2px
```
...
[![전체 아키텍처 시퀀스 다이어그램](full-sequence-diagram.png)](full-sequence-diagram.png)(이미지를 클릭하면 원본 크기로 볼 수 있습니다.)
